/*
  Model Rocket sonic locator trigger by an altimeter Ver 1.0
 Copyright Boris du Reau 2012-2017
 
 This is using a BMP085 pressure sensor and an Attiny 85
 The following should start a Sonic locator 30m above ground of the launch site
...
const int pinPyroOut = 3;
const int pinPyroOutContinuity = 4;
...
void setup()
{
  ...
  pinMode(pinPyroOut, OUTPUT);
  pinMode(pinSpeaker, OUTPUT);
  pinMode(pinPyroOutContinuity, INPUT);
  //Make sure that the output are turned off
  digitalWrite(pinPyroOut, LOW);
...
void loop()
{
  ...
  {
    continuityCheck(pinPyroOutContinuity);
  }
  ...
  if(currAltitude > liftoffAltitude)
  {
    ...
    if (currAltitude < lastAltitude)
    {
      ...
      if (measures == 0)
      {
        ...
        //just record that we have reach apogee
        apogeeHasFired=true;
        //record the apogee altitude
  ...
  if ((currAltitude  < 30) && apogeeHasFired == true && buzzerHasFired==false)
  {
    // Start sonic locator  30m before landing...
    digitalWrite(pinPyroOut, HIGH);
    buzzerHasFired=true;
    
  }
  if(apogeeHasFired == true && buzzerHasFired==true)
  {
